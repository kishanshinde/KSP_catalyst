...
async function generatePDFFromDB(body, req, res, resolve) {
    try {
        ...
        // Parse conversation messages and normalize format
        let rawMessages = [];
        if (record.conversation) {
            try {
                rawMessages = JSON.parse(record.conversation);
            } catch (e) {
        ...
        doc.registerFont(
    "KSP-Regular",
    path.join(__dirname, "fonts", "NotoSansKannada-Regular.ttf")
);

doc.registerFont(
    "KSP-Bold",
    path.join(__dirname, "fonts", "NotoSansKannada-Bold.ttf")
);
        const chunks = [];
        ...
        doc.fontSize(18).font('KSP-Bold');
        doc.text('KARNATAKA STATE POLICE', { align: 'center' });
        doc.fontSize(10).font('KSP-Regular');
        ...
        // Title
        doc.fontSize(14).font('KSP-Bold');
        doc.text(title);
        doc.moveDown(0.3);

        // Date
        doc.fontSize(9).font('KSP-Regular');
        ...
        doc.moveTo(50, doc.y).lineTo(545, doc.y).stroke('#cccccc');
        doc.moveDown(0.3);
        doc.fontSize(8).font('KSP-Regular').fillColor('#666666');
